...
for i=1:numBonds
    ...
    end_x=N(endnode).x;
    end_y=N(endnode).y;
    line([start_x,end_x],[start_y,end_y],'LineStyle','-','LineWidth',1.5,'Color','r')
end
%-----------------------------------------------
...
%----plot the attached fiber to the bead if there is a bead-----
 if isfield(I,'Bb')
   for i=1:length(I.Bb)
       ...
       line([x_inner,x_outer],[y_inner,y_outer],'LineStyle','-','LineWidth',1.5,'Color','r');  
   end
 end
%---------------------------------------------------------------
