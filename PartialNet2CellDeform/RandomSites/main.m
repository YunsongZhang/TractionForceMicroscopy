...
%-----Insert the bead in and revise the network-----------------------------
 InitialNet=put_bead_in(BlankNet,SimuSetting);      %something wrong here%%%%%%%%%%%%
...
%-----enlarge the bead in case some attached fiber is too short to cause numeric singularity-----
    while any(cell2mat({InitialNet.L0})<5e-2)
        SimuSetting.R=SimuSetting.R+1e-2;
        InitialNet=put_bead_in(BlankNet,SimuSetting);
        %flag=flag+1;
...
compare=[0.5*ones(length(Angle),1);Angle'];
[compare,Optim_result']
